...
if (hasPf) shots.push(
  ...
  { name: "30-pf-folder-isolate", caption: "Pathfinder · colour by folder, then click 'Equipment' in the legend → isolates just that folder (dims the rest). The real answer to 34-folders-8-colours.", map: "pathfinder.eido", vp: DESKTOP, setup: async (p) => { await setControl(p, "color", "meta:folder"); await settle(p, 600); await p.locator('[role="button"][aria-label="isolate folder Equipment"]').click(); await settle(p, 600); } },
