...
const shots: Shot[] = [
  ...
  { name: "26-grain-mid", caption: "Grain — a middle level of the ladder", vp: DESKTOP, setup: async (p) => { const gm = await grainMax(p); await setGrain(p, Math.max(1, Math.round(gm / 2))); } },
  ...
  { name: "28-drill", caption: "Double-click drills a region to a finer grain + fits to it", vp: DESKTOP, setup: async (p) => { const c = await p.evaluate(() => { const cv = document.querySelector("canvas")!; const r = cv.getBoundingClientRect(); return [r.left + r.width / 2, r.top + r.height / 2]; }); await p.mouse.dblclick(c[0], c[1]); } },
