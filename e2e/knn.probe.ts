...
import { rowDefects, strictRecall as recall } from "../src/knn/recall";
...
// recall is the SHARED strict implementation (src/knn/recall.ts) — n×K denominator + row-shape
...
(window as any).runKnnProbe = async () => {
  ...
  out.seamRowDefects = rowDefects(seam.idx, n, K);
  out.seamRecall = +recall(seam.idx, truth.idx, K).toFixed(4);
  t0 = performance.now();
  const wasm = await hnswWasmKnn(X, K, SEED);
  out.wasmMs = Math.round(performance.now() - t0);
  out.wasmRowDefects = rowDefects(wasm.idx, n, K);
  out.wasmRecall = +recall(wasm.idx, truth.idx, K).toFixed(4);
  ...
  const nbrRows = D.nbr.map((r: number[], i: number) => [i, ...r]);
  out.engineNbrBadRows = exactUpToTies(Xc, nbrRows, truthC.idx, 8);
  out.engineNbrRowDefects = rowDefects(nbrRows, nDocs, 8);
  out.engineNbrRecall = +recall(nbrRows, truthC.idx, 8).toFixed(4);
  return out;
};
