...
// COLOR COORDINATES (eid-zsij): a DEDICATED 2D projection of the card vectors onto the unit disc —
// the view-independent substrate every data colour derives from (region hue = member-centroid angle,
// categorical value hue = the same over its members, bipolar ramp poles = decile centroids). Same
// library and params family as the layout, over the SAME precomputed kNN graph, but its OWN seed
// stream: colour must not simply repaint the xy layout (it has to survive view switches to axis
// scatter/3D honestly), and reusing SEED would make it exactly the xy fit.
export const COLOR_SEED = SEED + 1;
// Normalize a raw 2D embedding to the unit disc: centre on the mean, scale so the 90th-percentile
// radius = 1 (clamped) — the disc the viewer maps onto the theme's OKLCH hue ring. Percentile, not
// max, so a single outlier can't compress everyone else into the achromatic centre.
export function toUnitDisc(pts: number[][]): number[][] {
  const n = pts.length;
  if (!n) return [];
  const mx = pts.reduce((s, p) => s + p[0], 0) / n, my = pts.reduce((s, p) => s + p[1], 0) / n;
  const rs = pts.map((p) => Math.hypot(p[0] - mx, p[1] - my)).sort((a, b) => a - b);
  const r90 = rs[Math.floor(0.9 * (n - 1))] || 1;
  return pts.map((p) => {
    const dx = p[0] - mx, dy = p[1] - my, r = Math.hypot(dx, dy);
    const s = r > r90 ? r90 / r : 1;
    return [+((dx * s) / r90).toFixed(4), +((dy * s) / r90).toFixed(4)];
  });
}

// Seeded 2-component PCA of the card vectors — the colour projection below UMAP's neighbor floor
// (n < 5). Same contract as the UMAP colour fit: DERIVED FROM THE VECTORS (identical embeddings →
// identical coords, different embeddings → different coords), deterministic (seeded power-iteration
// init; sign fixed by the largest-|loading| component). Trivial cost: the Gram matrix is n×n, n ≤ 4.
export function pca2(X: number[][], seed: number): number[][] {
  const n = X.length;
  if (!n) return [];
  if (n === 1) return [[0, 0]];
  const d = X[0].length;
  ...
  const G = C.map((a) => C.map((b) => a.reduce((s, v, j) => s + v * b[j], 0)));   // n×n Gram
  const rnd = mulberry32(seed);
  const scores: number[][] = [];
  for (let comp = 0; comp < 2; comp++) {
    let u = Array.from({ length: n }, () => rnd() - 0.5);
    for (const p of scores) { const dot = u.reduce((s, v, i) => s + v * p[i], 0), pp = p.reduce((s, v) => s + v * v, 0) || 1; u = u.map((v, i) => v - (dot / pp) * p[i]); }
    let lam = 0;
    for (let it = 0; it < 200; it++) {
      let w = G.map((row) => row.reduce((s, v, i) => s + v * u[i], 0));
      for (const p of scores) { const dot = w.reduce((s, v, i) => s + v * p[i], 0), pp = p.reduce((s, v) => s + v * v, 0) || 1; w = w.map((v, i) => v - (dot / pp) * p[i]); }
      lam = Math.hypot(...w);
      if (lam < 1e-12) { u = u.map(() => 0); break; }   // no variance left: inert component
      u = w.map((v) => v / lam);
    }
    // PCA scores along this component are sqrt(λ)·u (u is a unit eigenvector of the Gram matrix);
    // fix the sign deterministically: the largest-|value| entry (lowest index on ties) is positive.
    let m = 0; for (let i = 1; i < n; i++) if (Math.abs(u[i]) > Math.abs(u[m])) m = i;
    const sgn = u[m] < 0 ? -1 : 1, sc = sgn * Math.sqrt(lam);
    scores.push(u.map((v) => v * sc));
  }
  return Array.from({ length: n }, (_, i) => [scores[0][i], scores[1][i]]);
}
...
export async function projectAndCluster(embs: number[][], opts: { knn?: Knn; layoutApprox?: LayoutKnnApprox } = {}) {
  ...
  if (n < 5) { // too few points for UMAP/clustering — lay them on a ring so the tool still runs.
    // Colour coordinates stay a REAL projection of the card vectors (seeded PCA — deterministic,
    // vector-derived, trivial at this n): the format's "dedicated projection of card vectors"
    // claim holds unconditionally; only the LAYOUT falls back to the index ring.
    const xy = X.map((_, i) => [Math.cos((2 * Math.PI * i) / n) * 0.6, Math.sin((2 * Math.PI * i) / n) * 0.6] as number[]);
    const one = X.map(() => 0);
    return { xy, xyz: xy.map((p) => [p[0], p[1], 0]), xyzAgree: n > 1 ? Math.min(8, n - 1) : 0, cluster: one, k: 1, di: 0, levels: [one], counts: [1], hub: X.map(() => 0), nbr: X.map(() => [] as number[]), knnMethod: "none", colorCoords: toUnitDisc(pca2(X, COLOR_SEED)) };
  }
  ...
  const fitUMAP = (nComponents: number, seed = SEED) => {
  ...
  // the colour projection: same family (UMAP-2D over the same graph), its own seed → its own view
  const colorCoords = toUnitDisc(fitUMAP(2, COLOR_SEED));
  ...
  return { xy, xyz, xyzAgree: xyzOverlap(xy, xyz, K, opts.layoutApprox), cluster, k, di, levels, counts, hub, nbr, knnMethod: pre.method, colorCoords };
}
