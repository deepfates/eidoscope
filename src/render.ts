...
<div id="intro"><div class="box"><h2>the forms of the corpus 🔭</h2><div class="sub" id="introsub"></div><ul><li><b>Proximity is similarity</b> — nearby cards are alike; color is an emergent region, size is influence.</li><li><b>Slide the grain</b> to move the regions from continents to towns; <b>click a region</b> (or double-click the map) to drill in.</li><li><b>Click any card</b> to open the source, read its summary, and see where it sits on the axes.</li><li><b>Open the deck</b> to read the corpus as a sortable list — or set <b>layout → axis scatter</b> to position by any two discovered axes.</li></ul><button id="introgo">explore →</button></div></div>
...
function setGrain(L){if(!levels.length)return;L=Math.max(0,Math.min(levels.length-1,L));grain=L;const asg=levels[L],lab=levelLabels[L]||[];const cnt={};for(const n of nodes){n.cl=asg[n.i];cnt[n.cl]=(cnt[n.cl]||0)+1}k=counts[L];clusters=Array.from({length:k},(_,c)=>({c,n:cnt[c]||0,label:lab[c]||('region '+c),blurb:(levelBlurbs[L]||[])[c]||''}));buildLegend();draw()}
