...
export function buildReport(D: MapContract, name = "Corpus", opts: { usage?: string } = {}): string {
  ...
  // One decimal normally — but never let rounding contradict the 2% main/minor split: a 1.96% minor
  // axis must not print as "2.0%" under a header saying "each under 2%". Add decimals until the
  // printed number sits on the same side of 2 as the flag that grouped it.
  const pct = (a: typeof D.axes[number]) => {
    if (a.variance == null) return "";
    let d = 1, s = (a.variance * 100).toFixed(d);
    while (a.weak && Number(s) >= 2 && d < 6) s = (a.variance * 100).toFixed(++d);
    return ` — _${s}% of the variation_`;
  };
  out.push(`\n## How it varies\n`);
  ...
  out.push(`\n---`);
  if (opts.usage) out.push(`_${opts.usage}_\n`);
  out.push(`_Generated by [eidoscope](https://github.com/deepfates/eidoscope). Open the \`.html\` for the interactive map, or the \`.eido\` in the viewer._`);
  return out.join("\n") + "\n";
}
