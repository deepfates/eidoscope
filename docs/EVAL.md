...
|---|---|---|---|
| pitchfork (19,299) | **genre** | Pitchfork's editorial genre tag (9 values, multi) | Coarse (baseline 0.37) and topical: genre words are in the prose *and* in the source frontmatter the card model read. The friendliest verifier here. |
| pitchfork | **artist** | same-artist pairs (9,360 artists) | Very sparse → baseline 0.0014, so lifts are large by construction. Rewards proper-noun retention as much as musical relatedness (the artist's name is in the review). |
| pitchfork | **author** | same reviewer | A staffing fact, not a music fact — but critics have beats, so it partly re-measures genre and era. Also in the frontmatter. |
