...
{#snippet rightControls(scope: string)}
  <button class="btn btn-sm btn-ghost flex-none normal-case" title="read the corpus as a sortable, filterable list" onclick={() => (m.deckOpen = true)}>deck</button>
  <button disabled={m.channels.color !== "region"} title={m.channels.color !== "region" ? "region labels show when coloured by region" : "show region labels on the map"}
    aria-pressed={labelsOn} class="btn btn-sm flex-none normal-case {m.showLabels && m.channels.color === 'region' ? 'btn-active' : 'btn-ghost'}" onclick={() => (m.showLabels = !m.showLabels)} aria-label="toggle region labels">region labels</button>
