...
{#snippet controls(scope: string)}
  ...
  {#if nLevels > 1}
    <label class="flex flex-none items-center gap-1.5 px-1 text-xs {foldCls(scope, 4)}" data-fold="4" title="how finely the map is divided into regions — continents to towns">
  ...
  {#if scrubFields.length && scrubRange && scrubField}
    <Popover.Root>
      <Popover.Trigger class="btn btn-sm btn-ghost flex-none gap-1 normal-case {foldCls(scope, 1)}" data-fold="1" data-menu="{scope}:window" aria-label="window a dimension — currently {scrubField.name}" title="window the corpus along {scrubField.name}">
  ...
  {#if store?.vectors()}
    <Popover.Root>
      <Popover.Trigger class="btn btn-sm btn-ghost flex-none gap-1 normal-case {foldCls(scope, 1)}" data-fold="1" data-menu="{scope}:axis" aria-label="add an axis from a question" title="add an axis from a question you ask the corpus">
...
{#snippet rightControls(scope: string)}
  <button class="btn btn-sm btn-ghost flex-none normal-case {foldCls(scope, 3)}" data-fold="3" title="read the corpus as a sortable, filterable list (d)" onclick={() => (m.deckOpen = true)}>deck</button>
  <button disabled={m.channels.color !== "region"} title={m.channels.color !== "region" ? "region labels show when coloured by region" : "show region labels on the map (l)"}
    aria-pressed={labelsOn} data-fold="1" class="btn btn-sm flex-none normal-case {m.showLabels && m.channels.color === 'region' ? 'btn-active' : 'btn-ghost'} {foldCls(scope, 1)}" onclick={() => (m.showLabels = !m.showLabels)} aria-label="toggle labels">labels</button>
  <button class="btn btn-sm btn-ghost btn-square flex-none {foldCls(scope, 1)}" data-fold="1" onclick={toggleTheme} aria-label="toggle light or dark theme" title="toggle light / dark">{theme === "dark" ? "☾" : "☀"}</button>
  <DropdownMenu.Root>
    <DropdownMenu.Trigger class="btn btn-sm btn-ghost flex-none gap-1 normal-case {foldCls(scope, 1)}" data-fold="1" data-menu="{scope}:theme" aria-label="pick a colour theme" title="theme">
  ...
  <button class="btn btn-sm btn-ghost flex-none normal-case {foldCls(scope, 2)}" data-fold="2" title="clear every filter, close the open card, and return grain + camera to this map's defaults (r)" onclick={reset}>reset view</button>
{/snippet}
