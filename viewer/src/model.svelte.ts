...
export class ViewModel {
  ...
  snapshot(): ViewState {
    ...
    if (this.citeOn) s.cite = true;
    if (this.ghostsOn) s.ghosts = true;
    if (!this.showLabels) s.labels = false;
    if (this.deckOpen) s.deckOpen = true;
    if (this.deckQ) s.deckQ = this.deckQ;
    if (this.deckUnread) s.deckUnread = true;
    return s;
  }
  ...
  serializeUrl(pathname: string, search: string): string {
    ...
    if (ch.sort && ch.sort !== "hub") p.set("sort", ch.sort);   // the sort channel serializes like the other six
  ...
  applyPatch(p: StatePatch) {
    ...
    if (p.deckOpen) this.deckOpen = true;
