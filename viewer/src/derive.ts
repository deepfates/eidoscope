...
export function deriveDirection(vectors: number[][] | undefined, selected: number[]): Float32Array | null {
  if (!vectors?.length || !selected?.length) return null;
  const n = vectors.length, dim = vectors[0]?.length ?? 0;
  if (!dim) return null;
  ...
  for (let i = 0; i < n; i++) {
    const v = vectors[i]; if (!v || v.length !== dim) continue;
    let nv = 0; for (let j = 0; j < dim; j++) nv += v[j] * v[j];
    nv = Math.sqrt(nv) || 1;                       // L2-normalize each card first, so long cards don't dominate
    const t = sel.has(i) ? inM : outM;
    for (let j = 0; j < dim; j++) t[j] += v[j] / nv;
  }
