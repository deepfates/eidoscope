...
const N = 40, DIM = 6;
const cv = (rows: number[][]) => ({ data: Float32Array.from(rows.flat()), dim: rows[0]?.length ?? 0 });
const rows = Array.from({ length: N }, (_, i) => {
  const a = i < N / 2;
  return Array.from({ length: DIM }, (_, j) => (j === (a ? 0 : 1) ? 1 : 0) + 0.1 * Math.sin(i * 1.3 + j));
});
const vectors = cv(rows);
...
test("a card's own length can't buy it a direction — vectors are unit-normalized first", () => {
  const scaled = cv(rows.map((v, i) => (i < N / 2 ? v.map((x) => x * 7) : v)));
...
test("no honest direction → null (no vectors, empty set, the WHOLE corpus, or coincident centroids)", () => {
  ...
  expect(deriveDirection(cv([]), A)).toBeNull();
  ...
  const flat = cv(Array.from({ length: 10 }, () => [1, 0, 0]));  // every card identical → no discriminant
